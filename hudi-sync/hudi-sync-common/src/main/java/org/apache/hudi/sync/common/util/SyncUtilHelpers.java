...
public class SyncUtilHelpers {
  ...
  static AbstractSyncTool instantiateMetaSyncTool(String syncToolClassName,
                                                 TypedProperties props,
                                                 Configuration hadoopConfig,
                                                 FileSystem fs,
                                                 String targetBasePath,
    ...
    if (ReflectionUtils.hasConstructor(syncToolClassName,
      ...
      return ((AbstractSyncTool) ReflectionUtils.loadClass(syncToolClassName,
          new Class<?>[] {TypedProperties.class, Configuration.class, FileSystem.class},
          properties, hadoopConfig, fs));
    } else {
      LOG.warn("Falling back to deprecated constructor for class: " + syncToolClassName);
      try {
        return ((AbstractSyncTool) ReflectionUtils.loadClass(syncToolClassName,
            new Class<?>[] {Properties.class, FileSystem.class}, properties, fs));
      } catch (Throwable t) {
        throw new HoodieException("Could not load meta sync class " + syncToolClassName, t);
      }
    }
  }
}
